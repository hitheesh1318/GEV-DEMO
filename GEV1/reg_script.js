...
async function submitToBackend(event) {
  ...
  if (!validateForm(form)) {
    console.warn('Form validation failed - check that all required fields are filled');
    return;
  }
  ...
  try {
    ...
    if (resp.status === 409) {
      // Duplicate empId
      alert(`Employee ID "${payload.empId}" already exists. Please use a unique ID.`);
      return;
    }
...
window.addEventListener('DOMContentLoaded', () => {
  ...
  document.getElementById('btnExcel')?.addEventListener('click', exportExcelCSV);
  document.getElementById('btnXlsx') ?.addEventListener('click', exportExcelXLSX);
  document.getElementById('employeeForm')?.addEventListener('submit', submitToBackend);
});